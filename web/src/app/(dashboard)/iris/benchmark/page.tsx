    ...
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-6xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-300 to-purple-300 bg-clip-text text-transparent">IRIS&trade; Benchmark</h1>
          <p className="text-gray-400 mt-1">Cross-organizational comparison</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
            <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Your Position</h2>
            {position ? (
              <div className="space-y-4">
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-500 mt-1">of {position.total} organizations</div>
                </div>
            ...
            ) : <div className="text-gray-500 text-center py-8">No benchmark data</div>}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
            <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Dimension Comparison</h2>
            {radarData.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <RadarChart data={radarData}>
                  ...
                  <Radar name="Your Organization" dataKey="you" stroke="#818CF8" fill="#818CF8" fillOpacity={0.15} />
                  <Radar name="Peer Average" dataKey="avg" stroke="#F472B6" fill="#F472B6" fillOpacity={0.1} />
                </RadarChart>
              </ResponsiveContainer>
            ) : <div className="text-gray-500 text-center py-8">No peer data available for comparison</div>}
          </motion.div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
          <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Filter Comparison</h2>
          <div className="flex gap-4 items-end">
            ...
            <button onClick={compare} className="px-6 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg text-sm font-medium hover:from-indigo-500 hover:to-purple-500 transition-all">Compare</button>
          </div>
          {comparison && (
            <div className="mt-4 text-sm text-gray-400">
              Comparing against <span className="text-white font-medium">{comparison.peerCount}</span> peer organizations
              {comparison.industryAverage != null && <> · Industry average: <span className="text-white font-medium">{comparison.industryAverage.toFixed(0)}</span></>}
              {comparison.percentile != null && <> · You are in the <span className="text-indigo-400 font-medium">{comparison.percentile}th</span> percentile</>}
            </div>
          )}
        </motion.div>
      </div>
