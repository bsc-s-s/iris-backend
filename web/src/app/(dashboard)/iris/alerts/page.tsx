    ...
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-300 to-purple-300 bg-clip-text text-transparent">{t('alerts.title')}</h1>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <span className={`text-[10px] uppercase px-2 py-0.5 rounded ${a.severity === 'critical' ? 'bg-red-900/40 text-red-300' : a.severity === 'high' ? 'bg-orange-900/40 text-orange-300' : a.severity === 'medium' ? 'bg-yellow-900/40 text-yellow-300' : 'bg-gray-700 text-gray-400'}`}>{t(`severity.${a.severity}`)}</span>
                    ...
                    {a.metric && <div className="text-xs text-gray-500 mt-2">{t('alerts.metric')}: {a.metric} · {t('alerts.current')}: {a.currentValue} · {t('alerts.threshold')}: {a.threshold}</div>}
                  </div>
                  <button onClick={() => dismiss(a.id)} disabled={dismissing === a.id} className="px-4 py-2 border border-gray-700 rounded-lg text-xs hover:bg-gray-700 transition-all disabled:opacity-50 whitespace-nowrap">
                    {dismissing === a.id ? '...' : t('alerts.dismiss')}
                  </button>
                </div>
              </motion.div>
            ))}
