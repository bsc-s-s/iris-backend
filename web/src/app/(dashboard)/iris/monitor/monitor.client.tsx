...
export function MonitorClient({ initialStatus, initialCycles }: { initialStatus: any; initialCycles: any[] }) {
  const { t } = useI18n();
  const [status, setStatus] = useState<any>(initialStatus);
  const [cycles, setCycles] = useState<any[]>(initialCycles);
  ...
  async function runCycle(freq: string) {
    setRunning(true);
    try { await iris.monitor.run(freq); await load(); } catch {}
    setRunning(false);
  }

  const freqLabel = (f: string) => f === 'weekly' ? t('monitor.run_weekly') : f === 'monthly' ? t('monitor.run_monthly') : t('monitor.run_quarterly');

  return (
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-300 to-purple-300 bg-clip-text text-transparent">{t('monitor.title')}</h1>
          <p className="text-gray-400 mt-1">{t('monitor.subtitle')}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
          <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">{t('monitor.status')}</h2>
          {status ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-gray-800/50 rounded-lg p-4 text-center">
                ...
                <div className="text-xs text-gray-500 mt-1">{t('monitor.status')}</div>
              </div>
              <div className="bg-gray-800/50 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold">{status.daysSinceLastCycle ?? '—'}</div>
                <div className="text-xs text-gray-500 mt-1">{t('monitor.days_since')}</div>
              </div>
              <div className="bg-gray-800/50 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold">{status.lastScore?.overallScore ?? '—'}</div>
                <div className="text-xs text-gray-500 mt-1">{t('monitor.current_score')}</div>
              </div>
              <div className="bg-gray-800/50 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold capitalize">{status.lastScore?.classification ? t(`class.${status.lastScore.classification}`) : '—'}</div>
                <div className="text-xs text-gray-500 mt-1">{t('monitor.classification')}</div>
              </div>
            </div>
        ...
        <div className="flex gap-4">
          {['weekly', 'monthly', 'quarterly'].map(freq => (
            <button key={freq} onClick={() => runCycle(freq)} disabled={running} className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl text-sm font-medium hover:from-indigo-500 hover:to-purple-500 transition-all disabled:opacity-50 capitalize">
              {running ? t('monitor.running') : freqLabel(freq)}
            </button>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
          <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">{t('monitor.cycle_history')}</h2>
          {cycles.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-gray-500 text-xs uppercase border-b border-gray-800"><th className="text-left py-2 pr-4">{t('monitor.frequency')}</th><th className="text-left py-2 pr-4">{t('monitor.status_col')}</th><th className="text-left py-2 pr-4">{t('monitor.started')}</th><th className="text-left py-2 pr-4">{t('monitor.signals_col')}</th><th className="text-left py-2">{t('monitor.alerts_col')}</th></tr></thead>
          ...
          ) : <div className="text-gray-500 text-center py-8">{t('monitor.no_cycles')}</div>}
        </motion.div>
      </div>
    </div>
  );
