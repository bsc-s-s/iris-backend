      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-300 to-purple-300 bg-clip-text text-transparent">IRIS&trade; Intelligence</h1>
          </div>
          <p className="text-gray-400">Organizational Intelligence &amp; Risk Platform</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className="relative overflow-hidden rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-8">
          ...
          <div className="relative flex items-center gap-8 flex-wrap">
            ...
            <div className="flex-1 min-w-[200px]">
              <div className="text-xs text-gray-500 uppercase tracking-wider mb-2">Ten-Dimensional Score</div>
            ...
            {score?.confidence && <div className="text-xs text-gray-500">Confidence: {(score.confidence * 100).toFixed(0)}%</div>}
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-2 rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
            <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Organizational Radar</h2>
            {radar.length > 0 ? (
            ...
            ) : <div className="text-gray-500 text-center py-12">Complete an IRIS Scan to see your organizational radar</div>}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
            <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Active Alerts</h2>
            {alerts.length > 0 ? (
                ...
                {alerts.map((a: any) => (
                  <div key={a.id} className={`p-3 rounded-lg border ${a.severity === 'critical' ? 'border-red-800 bg-red-900/20' : a.severity === 'high' ? 'border-orange-800 bg-orange-900/20' : 'border-gray-700 bg-gray-800/50'}`}>
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`text-[10px] uppercase px-2 py-0.5 rounded ${a.severity === 'critical' ? 'bg-red-900/40 text-red-300' : a.severity === 'high' ? 'bg-orange-900/40 text-orange-300' : 'bg-gray-700 text-gray-400'}`}>{a.severity}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
            <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Score Trend</h2>
            ...
            ) : <div className="text-gray-500 text-center py-8">Insufficient data for trend</div>}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
            <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Predictive Risk Models</h2>
                ...
                {predictions.slice(0, 5).map((p: any) => (
                  <div key={p.id} className="flex items-center gap-4">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-500">{p.timeHorizon} &middot; {(p.probability * 100).toFixed(0)}% probability</div>
                    </div>
            ...
            ) : <div className="text-gray-500 text-center py-8">Run predictions to see risk models</div>}
          </motion.div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
          <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Recent Risk Signals</h2>
          {signals.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>{signals.slice(0, 10).map((s: any) => (
                  <tr key={s.id} className="border-b border-gray-800/50 hover:bg-gray-800/30"><td className="py-3 pr-4">{s.title}</td><td className="py-3 pr-4 capitalize">{s.category}</td><td className="py-3 pr-4"><span className={`text-[10px] uppercase px-2 py-0.5 rounded ${s.severity === 'critical' ? 'bg-red-900/40 text-red-300' : s.severity === 'high' ? 'bg-orange-900/40 text-orange-300' : s.severity === 'medium' ? 'bg-yellow-900/40 text-yellow-300' : 'bg-gray-700 text-gray-400'}`}>{s.severity}</span></td><td className="py-3">{(s.confidence * 100).toFixed(0)}%</td></tr>
                ))}</tbody>
              </table>
            </div>
          ) : <div className="text-gray-500 text-center py-8">No risk signals detected yet</div>}
        </motion.div>

        {/* Quick Actions */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} className="flex flex-wrap gap-4">
          <a href="/iris/scan" className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl text-sm font-medium hover:from-indigo-500 hover:to-purple-500 transition-all">Start New Scan</a>
          <a href="/iris/reports" className="px-6 py-3 border border-gray-700 rounded-xl text-sm font-medium hover:bg-gray-800 transition-all">View Reports</a>
          <a href="/iris/benchmark" className="px-6 py-3 border border-gray-700 rounded-xl text-sm font-medium hover:bg-gray-800 transition-all">Benchmark</a>
        </motion.div>
      </div>
    </div>
  );
