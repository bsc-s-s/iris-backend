    ...
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-6xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-300 to-purple-300 bg-clip-text text-transparent">IRIS&trade; Predict</h1>
          <p className="text-gray-400 mt-1">Predictive risk models — AI-driven foresight</p>
        </motion.div>
        ...
        <button onClick={runPredictions} disabled={running} className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl text-sm font-medium hover:from-indigo-500 hover:to-purple-500 transition-all disabled:opacity-50">
          {running ? 'Running Predictions...' : 'Run All Predictions'}
        </button>
        ...
        {loading ? (
          ...
          <>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
              <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4">Probability Overview</h2>
                ...
                <motion.div key={p.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-gray-800 bg-gray-900/50 backdrop-blur-xl p-6">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">{p.timeHorizon} horizon</div>
                    </div>
                  ...
                  {p.causalFactors?.length > 0 && (
                    <div className="mb-3">
                      <div className="text-xs text-gray-500 uppercase mb-1">Causal Factors</div>
                  ...
                  {p.recommendations?.length > 0 && (
                    <div>
                      <div className="text-xs text-gray-500 uppercase mb-1">Recommendations</div>
          ...
          <div className="text-center py-16 text-gray-500">
            ...
            <p>Run predictions to see risk forecasts</p>
          </div>
        )}
      </div>
    </div>
