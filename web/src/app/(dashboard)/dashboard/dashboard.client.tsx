...
export function ExecutiveDashboardClient({ initialData }: { initialData: any }) {
  ...
  const {
    ...
    recentAssessments: initialRecentAssessments,
  } = initialData;

  const [reportLoading, setReportLoading] = useState(false);
  const [reportData, setReportData] = useState<any>(null);
  const [auditLogs] = useState(initialAuditLogs || []);
  const [recentAssessments] = useState(initialRecentAssessments || []);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <GlassCard className="lg:col-span-2">
          ...
          <HeatmapGrid data={heatmapData} loading={false} />
        </GlassCard>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <GlassCard>
          ...
          <Timeline events={timelineEvents} loading={false} />
        </GlassCard>
        ...
        <GlassCard>
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-xs text-iris-400">{recentAssessments.length} registradas</p>
            </div>
