...
async function bootstrap() {
  ...
  // Proxy non-API requests to the frontend (catch-all after NestJS routes)
  server.all('*', async (req: any, res: any, next: any) => {
    if (req.path.startsWith('/api') || req.path === '/' || req.path.startsWith('/.')) return next();
